...
def generate_graph(m_pre, m_post, output_name):
    ...
    bar1 = ax.bar([p - width/2 for p in x], pre_values, width,
                  label='PRE-TEST (Default)', color='#e74c3c')
    bar2 = ax.bar([p + width/2 for p in x], post_values, width,
...
======================================================================
    ...
    usage_example = """
Contoh Penggunaan:
  python analyzer_ta_suricata.py --pre pre.json --post post.json --attacks 70 --ip 192.168.100.10 --output hasil.xlsx
    """
    ...
    parser.add_argument("--pre", required=True,
                        help="Path ke file eve.json PRE-TEST [cite: 182]")
    parser.add_argument("--post", required=True,
                        help="Path ke file eve.json POST-TEST [cite: 186]")
    parser.add_argument("--attacks", type=int, required=True,
                        help="Total paket serangan yang dikirim [cite: 79, 321]")
    parser.add_argument("--ip", required=True,
                        help="IP Attacker/Kali Linux [cite: 274, 309]")
    parser.add_argument(
        "--plot", help="Nama file gambar grafik (contoh: grafik.png)")
    parser.add_argument(
        "-o", "--output", help="Simpan tabel analisis komparatif ke Excel (.xlsx)")

    args = parser.parse_args()

    df_pre_data = load_data(args.pre)
    df_post_data = load_data(args.post)

    if df_pre_data is None or df_post_data is None:
        print("[Error] Salah satu file log tidak ditemukan atau rusak.")
        return

    m_pre = calculate_metrics(df_pre_data, args.ip, args.attacks)
    m_post = calculate_metrics(df_post_data, args.ip, args.attacks)

    # Membuat DataFrame untuk tabel hasil komparatif [cite: 248]
    data = {
    ...
    # Tampilkan Kesimpulan Singkat [cite: 171]
    print("\n" + "-"*30)
    ...
    # Simpan ke Excel jika argumen --output digunakan
    if args.output:
        df_result.to_excel(args.output, index=False, engine='openpyxl')
        print(f"\n[OK] Tabel hasil komparatif disimpan ke: {args.output}")
